...
<center> <h1>Edit Web Series</h1></center>
    <div class="container mt-5">
        <form method="post" id="edit_form" action="{{ route('web.update', $webSeries->id) }}">
            @csrf
            @method('PUT')
           
            <div class="form-group">
                <label>Title</label>
                <input type="text" name="title" class="form-control" value="{{ $webSeries->title }}">
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <select name="theme_id" class="form-control">
                    ...
                    @foreach ($themes as $id => $name)
                        <option value="{{ $id }}" {{ $webSeries->theme_id == $id ? 'selected' : '' }}>{{ $name }}</option>
                    @endforeach
                </select>
            </div>
